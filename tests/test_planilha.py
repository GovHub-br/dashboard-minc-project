...
class TestLeitura(unittest.TestCase):
    def test_le_uma_tarefa(self):
        t = planilha("Alta,1 · Gestão,,,1.1.1,Fazer algo,,SNH,10/09/2026,Em andamento")
        self.assertEqual(len(t), 1)
        self.assertEqual(t[0]["cod"], "1.1.1")
    ...
    def test_ignora_linha_sem_tarefa_ou_sem_eixo(self):
        t = planilha(
            "Alta,1 · Gestão,,,1.1.1,Válida,,,,Não iniciado",
            "Alta,1 · Gestão,,,1.1.2,,,,,Não iniciado",
            "Alta,,,,9.9.9,Sem eixo,,,,Não iniciado",
        )
        self.assertEqual([x["cod"] for x in t], ["1.1.1"])
    ...
    def test_dependencias_separadas_por_ponto_e_virgula(self):
        self.assertEqual(conv.le_dependencias("1.1.1; 2.3.1"), ["1.1.1", "2.3.1"])
...
class TestRede(unittest.TestCase):
    def test_passos_de_uma_cadeia(self):
        t = por_cod(planilha(
            "Alta,1 · A,,,1.1.1,Primeira,,,,Não iniciado",
            "Alta,1 · A,,,1.1.2,Segunda,1.1.1,,,Não iniciado",
            "Alta,1 · A,,,1.1.3,Terceira,1.1.2,,,Não iniciado",
        ))
        self.assertEqual([t["1.1.1"]["passo"], t["1.1.2"]["passo"],
                          t["1.1.3"]["passo"]], [0, 1, 2])

    def test_travada_enquanto_predecessor_nao_conclui(self):
        t = por_cod(planilha(
            "Alta,1 · A,,,1.1.1,Primeira,,,,Em andamento",
            "Alta,1 · A,,,1.1.2,Segunda,1.1.1,,,Não iniciado",
        ))
        self.assertFalse(t["1.1.1"]["travada"])
        self.assertTrue(t["1.1.2"]["travada"])

    def test_pronta_quando_predecessor_concluiu(self):
        t = por_cod(planilha(
            "Alta,1 · A,,,1.1.1,Primeira,,,,Concluído",
            "Alta,1 · A,,,1.1.2,Segunda,1.1.1,,,Não iniciado",
        ))
        self.assertFalse(t["1.1.2"]["travada"])
        self.assertTrue(t["1.1.2"]["pronta"])
    ...
    def test_tarefa_sem_dependencia_nao_e_pronta(self):
        ...
        t = por_cod(planilha("Alta,1 · A,,,1.1.1,Solta,,,,Não iniciado"))
        self.assertFalse(t["1.1.1"]["pronta"])

    def test_segura_conta_descendentes_indiretos(self):
        t = por_cod(planilha(
            "Alta,1 · A,,,1.1.1,Raiz,,,,Não iniciado",
            "Alta,1 · A,,,1.1.2,Meio,1.1.1,,,Não iniciado",
            "Alta,1 · A,,,1.1.3,Folha,1.1.2,,,Não iniciado",
        ))
        self.assertEqual(t["1.1.1"]["segura"], 2)
        self.assertEqual(t["1.1.2"]["segura"], 1)
        self.assertEqual(t["1.1.3"]["segura"], 0)

    def test_dependencia_para_codigo_inexistente_e_ignorada(self):
        t = por_cod(planilha(
            "Alta,1 · A,,,1.1.1,Só,9.9.9,,,Não iniciado",
        ))
        self.assertEqual(t["1.1.1"]["depende_de"], [])
        self.assertFalse(t["1.1.1"]["travada"])

    def test_ciclo_nao_trava_o_conversor(self):
        t = por_cod(planilha(
            "Alta,1 · A,,,1.1.1,Uma,1.1.2,,,Não iniciado",
            "Alta,1 · A,,,1.1.2,Outra,1.1.1,,,Não iniciado",
        ))
...
class TestResumo(unittest.TestCase):
    def test_gargalo_e_a_nao_concluida_que_mais_segura(self):
        tarefas = planilha(
            ...
            "Alta,1 · A,,,1.1.2,Meio,1.1.1,,,Não iniciado",
            "Alta,1 · A,,,1.1.3,Folha,1.1.2,,,Não iniciado",
            "Alta,2 · B,,,2.1.1,Outra raiz,,,,Não iniciado",
            "Alta,2 · B,,,2.1.2,Dependente,2.1.1,,,Não iniciado",
        )
        r = conv.resume(tarefas)
        self.assertEqual(r["gargalo"]["cod"], "1.1.1")
        self.assertEqual(r["gargalo"]["segura"], 2)

    def test_concluida_nao_e_gargalo(self):
        tarefas = planilha(
            "Alta,1 · A,,,1.1.1,Raiz,,,,Concluído",
            "Alta,1 · A,,,1.1.2,Meio,1.1.1,,,Não iniciado",
            "Alta,1 · A,,,1.1.3,Folha,1.1.2,,,Não iniciado",
        )
        r = conv.resume(tarefas)
        self.assertEqual(r["gargalo"]["cod"], "1.1.2")

    def test_progresso_medio(self):
        tarefas = planilha(
            "Alta,1 · A,,,1.1.1,Uma,,,,Concluído",
            "Alta,1 · A,,,1.1.2,Outra,,,,Não iniciado",
        )
        self.assertEqual(conv.resume(tarefas)["progresso_medio"], 50.0)

    def test_conta_sem_prazo(self):
        tarefas = planilha(
            "Alta,1 · A,,,1.1.1,Uma,,,10/09/2026,Não iniciado",
            "Alta,1 · A,,,1.1.2,Outra,,,não definido ainda,Não iniciado",
        )
...
class TestAlertas(unittest.TestCase):
    def test_prazo_vencido_entra(self):
        tarefas = planilha(
            "Alta,1 · A,,,1.1.1,Atrasada,,,01/01/2026,Em andamento")
    ...
    def test_sem_prazo_nunca_entra_como_atrasada(self):
        ...
        tarefas = planilha(
            "Alta,1 · A,,,1.1.1,Sem data,,,não definido ainda,Em andamento")
        self.assertEqual(conv.monta_alertas(tarefas, "2026-09-04"), [])

    def test_concluida_com_prazo_vencido_nao_alerta(self):
        tarefas = planilha(
            "Alta,1 · A,,,1.1.1,Feita,,,01/01/2026,Concluído")
        self.assertEqual(conv.monta_alertas(tarefas, "2026-09-04"), [])

    def test_bloqueada_e_em_risco_entram(self):
        tarefas = planilha(
            "Alta,1 · A,,,1.1.1,Travada,,,,Bloqueado",
            "Alta,1 · A,,,1.1.2,Arriscada,,,,Em risco",
        )
...
class TestEixos(unittest.TestCase):
    def test_agrupa_e_calcula_progresso_por_eixo(self):
        tarefas = planilha(
            "Alta,1 · Gestão,,,1.1.1,Uma,,,,Concluído",
            "Alta,1 · Gestão,,,1.1.2,Outra,,,,Não iniciado",
            "Alta,3 · Patrimônio,,,3.1.1,Terceira,,,,Em andamento",
        )
...
class TestSaida(unittest.TestCase):
    def test_nenhum_campo_interno_vaza(self):
        tarefas = planilha(
            "Alta,1 · A,,,1.1.1,Uma,,,,Não iniciado",
            "Alta,1 · A,,,1.1.2,Outra,1.1.1,,,Não iniciado",
        )
        for t in tarefas:
...
class TestPlanilhaVazia(unittest.TestCase):
    ...
    def test_cadeia_de_um_passo(self):
        tarefas = planilha("Alta,1 · A,,,1.1.1,Só uma,,,,Não iniciado")
